...
async def calcom_webhook(
    ...
    try:
        ...
        if event_type == "BOOKING_CREATED":
            # Extract booking info
            booking = payload.get("bookingCreated", {})
            attendees = booking.get("attendees", [])
            
            if not attendees:
                return {"status": "success", "message": "No attendees found in booking"}
                
            # Get the first attendee (usually the customer)
            attendee = attendees[0]
            email = attendee.get("email")
            name = attendee.get("name")
            
            # Try to find UTM parameters in metadata or custom fields
            utm_source = None
            utm_campaign = None
            ...
            # Look for booking questions/responses
            responses = booking.get("responses", {})
            for key, value in responses.items():
                if "how did you hear" in key.lower() or "referral" in key.lower():
                    if not utm_source:
                        utm_source = value
